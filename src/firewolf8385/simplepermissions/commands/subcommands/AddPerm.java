...
public class AddPerm implements CommandExecutor
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        ...
        try
        {
            ...
            ChatUtils.chat(sender, settings.getConfig().getString("Messages.permissionAdded").replace("%group%", args[0]).replace("%permission%", args[1]));
        ...
        {
            ChatUtils.chat(sender, settings.getConfig().getString("Messages.errorAddingPermission").replace("%group%", args[0]).replace("%permission%", args[1]));
            e.printStackTrace();
        }
